...
export default function SupplierList() {
  ...
  const load = async () => {
    setLoading(true);
    try { const r = await api.get("/suppliers"); setData(r.data.data || []); }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showToast(editId ? "Supplier updated!" : "Supplier created!", "success"); setModal(false); load();
    } catch (e) { showToast(e?.response?.data?.message || "Error saving", "danger"); } finally { setSaving(false); }
  };

  const handleDelete = async (id) => { await api.delete(`/suppliers/${id}`); showToast("Deleted!", "success"); load(); };
  ...
  const handlePrint = () => printReport({
    title: "Suppliers",
    subtitle: `Generated ${new Date().toLocaleString("en-IN")} · ${filtered.length} supplier(s)`,
    sections: [{ columns: printColumns, rows: filtered, emptyMessage: "No suppliers found" }],
  });
  ...
  return (
    <>
      ...
      />
      <FormModal open={modal} title={editId ? "Edit Supplier" : "Add Supplier"} onClose={() => setModal(false)} onSubmit={handleSave} loading={saving} size="lg">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormField label="Company / Name" id="name" value={form.name} onChange={set("name")} required placeholder="e.g. ABC Traders" />
          <FormField label="Phone" id="phone" value={form.phone} onChange={set("phone")} placeholder="e.g. 9876543210" />
          <FormField label="Email" id="email" type="email" value={form.email} onChange={set("email")} placeholder="e.g. abc@supplier.com" />
